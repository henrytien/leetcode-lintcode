// Source : https://leetcode.com/problems/odd-even-jump/submissions/
// Author : henrytine
// Date   : 2020-07-10
 ...
 * 
 * 	During odd numbered jumps (ie. jumps 1, 3, 5, ...), you jump to the index j such that A[i] 
 * <= A[j] and A[j] is the smallest possible value.  If there are multiple such indexes j, you can 
 * only jump to the smallest such index j.
 * 	During even numbered jumps (ie. jumps 2, 4, 6, ...), you jump to the index j such that A[i] 
 ...
 * Output: 2
 * Explanation: 
 * From starting index i = 0, we can jump to i = 2 (since A[2] is the smallest among A[1], A[2], A[3], 
 * A[4] that is greater or equal to A[0]), then we can't jump any more.
 * From starting index i = 1 and i = 2, we can jump to i = 3, then we can't jump any more.
 * From starting index i = 3, we can jump to i = 4, so we've reached the end.
 * From starting index i = 4, we've reached the end already.
 * In total, there are 2 different starting indexes (i = 3, i = 4) where we can reach the end with 
 ...
 * in (A[2], A[3], A[4]) that is less than or equal to A[1].  A[3] is also the largest value, but 2 is 
 * a smaller index, so we can only jump to i = 2 and not i = 3.
 * 
