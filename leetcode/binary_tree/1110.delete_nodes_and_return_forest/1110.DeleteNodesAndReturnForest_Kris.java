// Source : https://leetcode.com/problems/delete-nodes-and-return-forest/
// Author : Kris
// Date   : 2020-08-28

/***************************************************************************************************** 
 *
 * Given the root of a binary tree, each node in the tree has a distinct value.
 * 
 * After deleting all nodes with a value in to_delete, we are left with a forest (a disjoint union of 
 * trees).
 * 
 * Return the roots of the trees in the remaining forest.  You may return the result in any order.
 * 
 * Example 1:
 * 
 * Input: root = [1,2,3,4,5,6,7], to_delete = [3,5]
 * Output: [[1,2,null,4],[6],[7]]
 * 
 * Constraints:
 * 
 * 	The number of nodes in the given tree is at most 1000.
 * 	Each node has a distinct value between 1 and 1000.
 * 	to_delete.length <= 1000
 * 	to_delete contains distinct values between 1 and 1000.
 ******************************************************************************************************/

/**
 * Definition for a binary tree node.
 * public class TreeNode {
 *     int val;
 *     TreeNode left;
 *     TreeNode right;
 *     TreeNode() {}
 *     TreeNode(int val) { this.val = val; }
 *     TreeNode(int val, TreeNode left, TreeNode right) {
 *         this.val = val;
 *         this.left = left;
 *         this.right = right;
 *     }
 * }
 */
class Solution {
    public List<TreeNode> delNodes(TreeNode root, int[] to_delete) {
        var result = new ArrayList<TreeNode>();
        if (root == null) {
            return result;
        }
        
        var toDelete = IntStream.of(to_delete).boxed().collect(Collectors.toSet());
        if (!toDelete.contains(root.val)) {
            result.add(root);
        }
        
        helper(root, null, toDelete, result);
        
        return result;
    }
    
    void helper(TreeNode root, TreeNode parent,
                Set<Integer> toDelete, List<TreeNode> result) {
        if (root == null) {
            return;
        }
        
        var left = root.left;
        var right = root.right;
        if (toDelete.contains(root.val)) {
            if (left != null && !toDelete.contains(left.val)) {
                result.add(left);
            }
            if (right != null && !toDelete.contains(right.val)) {
                result.add(right);
            }
            if (parent != null && parent.left == root) {
                parent.left = null;
            } else if (parent != null && parent.right == root) {
                parent.right = null;
            }
        }
        
        helper(left, root, toDelete, result);
        helper(right, root, toDelete, result);
    }
}