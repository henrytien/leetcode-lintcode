...
class Solution {
    ...
    boolean helper(int[] stones, int index, int units) {
        ...
        for (var i = index + 1; i < stones.length; i++) {
            if (stones[index] + units + 1 < stones[i]) {
                break;
            }
            
            for (var j = units - 1; j <= units + 1; j++) {
                if (j >= 0 && stones[index] + j == stones[i] && helper(stones, i, j)) {
                    result = true;
                    break Outter;
                }
            }
