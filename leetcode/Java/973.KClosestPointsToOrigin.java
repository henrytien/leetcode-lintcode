...
/* no sqrt to avoid percision loss,the data given is okay
 * the problem transforms into top K
 * 1. BFNRT
 * 2. partition 
 * 3. maxHeap and maintain it
 * TODO : learn muitiMap in Java 
 */
class Solution {
    public int[][] kClosest(int[][] points, int K) {
        ...
        for(int i = K; i < points.length; i++){
            if(getDistance(maxHeap.peek()) > getDistance(points[i])){
                maxHeap.poll();
                maxHeap.offer(points[i]);
            }
        }
